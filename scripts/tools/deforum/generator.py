...
def generate(args, return_latent=False, return_sample=False, return_c=False):
    ...
    sampler = PLMSSampler(st.session_state["model"]) if args.sampler == 'plms' else DDIMSampler(
    ...
    if args.init_latent is not None:
        ...
        with precision_scope("cuda"):
            init_latent = st.session_state["model"].get_first_stage_encoding(
                st.session_state["model"].encode_first_stage(args.init_sample))
        ...
        with precision_scope("cuda"):
            init_latent = st.session_state["model"].get_first_stage_encoding(
                st.session_state["model"].encode_first_stage(init_image))  # move to latent space
            ...
            with st.session_state["model"].ema_scope():
                for prompts in data:
                    ...
                    for i in range(len(samples)):
                        x_samples_ddim = st.session_state["model"].decode_first_stage(samples[i].unsqueeze(0))
